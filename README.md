...
一套面向 Codex 的产品开发插件，覆盖 PM 优先级判断、产品闭环、信息架构、UI 文案隔离、图标化交互、专业色彩排版、统一素材库、前后端契约、测试验证和上线流程，帮助避免杂乱页面、PRD 式文案和不成熟的 AI 默认 UI。

这是一个本地 Codex 插件，用于把产品从 idea 推到可实现、可检查、可上线的闭环流程。它重点解决三类问题:

- Codex 生成前端时把 PRD、内部策略和“我们要怎么做”写进用户界面。
- 功能入口没有按用户使用频率、用户价值、业务价值和操作风险排序。
- 页面缺少成熟产品的信息架构、按钮层级、状态和前后端契约。
- 2C 首屏不能让用户快速理解产品用途、看到最高频动作并立即开始。
- 色彩、排版、布局、图标和交互缺少统一设计系统，导致界面像拼贴 demo。
- logo、官网背景、图标、空状态和社交预览没有统一素材库。

## 包含能力

- `product-loop-development`: PM 优先级、产品闭环、动线模型、2C 首屏、前端成熟度、UI 文案隔离、视觉语言、专业色彩排版、素材库和 QA。
...
- 先输出产品定位、目标用户、功能优先级、动线模型、按钮摆放依据、2C 首屏策略和素材库计划。
- 按使用频率、用户价值、业务价值和风险安排功能入口。
- 页面标题、tab、按钮使用短而成熟的产品语言。
- 用图标、步骤、状态、badge、进度和交互表达流程，减少解释型文字。
- 先定义色彩 token、字体层级、布局网格和状态映射，再写页面。
- 为官网和 2C 产品规划统一 logo、背景、图标、空状态、社交图和产品截图。
